...
if (!isset($_SESSION['user_id']) || $_SESSION['role_name'] !== 'office_manager') {
...
<?php endif; ?>
    ...
    $displayed_ids = [];
    // جلب رئيس القسم (المشرف المباشر)
    if (!empty($supervisor_id)) {
        $supervisor_info_query = "SELECT u.id, u.full_name, r.role_name 
        FROM users u 
        JOIN roles r ON u.role_id = r.id 
        WHERE u.id = :supervisor_id AND u.is_active = 1";
        ...
        if ($supervisor && !in_array($supervisor['id'], $displayed_ids)) {
            $displayed_ids[] = $supervisor['id'];
            ?>
            <option value="<?php echo $supervisor['id']; ?>">
                <?php echo htmlspecialchars($supervisor['full_name']) ?> (رئيس القسم)
            </option>
            <?php
        }
    }

    // جلب موظفي الديوان في نفس القسم
    $board_roles_query = "SELECT id FROM roles WHERE role_name LIKE '%board%'";
    $board_roles_stmt = $db->prepare($board_roles_query);
    $board_roles_stmt->execute();
    $board_role_ids = $board_roles_stmt->fetchAll(PDO::FETCH_COLUMN);
    
    if (!empty($board_role_ids) && !empty($department_id)) {
        $board_role_ids_str = implode(',', $board_role_ids);
        $board_users_query = "SELECT u.id, u.full_name, r.role_name 
                               FROM users u 
                               JOIN roles r ON u.role_id = r.id 
                               WHERE u.department_id = :dept_id 
                               AND u.role_id IN ($board_role_ids_str) 
                               AND u.id != :current_user_id
                               AND u.is_active = 1
                               ORDER BY u.full_name";
        $board_users_stmt = $db->prepare($board_users_query);
        $board_users_stmt->execute([
            ':dept_id' => $department_id,
            ':current_user_id' => $user_id
        ]);
        $board_users = $board_users_stmt->fetchAll(PDO::FETCH_ASSOC);
        
        foreach ($board_users as $board_user) {
            if (!in_array($board_user['id'], $displayed_ids)) {
                $displayed_ids[] = $board_user['id'];
                ?>
                <option value="<?php echo $board_user['id']; ?>">
                    <?php echo htmlspecialchars($board_user['full_name']) ?> (الديوان)
                </option>
                <?php
            }
        }
